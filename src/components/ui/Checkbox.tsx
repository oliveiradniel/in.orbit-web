...
export function CheckboxItem(props: Checkbox.CheckboxProps) {
  return (
    <Checkbox.Root
      {...props}
      className="group flex w-full cursor-pointer items-center justify-between rounded-lg border border-zinc-900 bg-black px-4 py-2.5 ring-pink-500/10 transition-colors duration-300 ease-linear outline-none hover:border-zinc-800 focus-visible:border-pink-500 focus-visible:ring-4 data-[state=checked]:border-pink-500 data-[state=checked]:bg-pink-500/5"
    >
      {props.children}
    </Checkbox.Root>
  );
